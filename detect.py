...
def run_program_after_delay(frame_center_x, frame_center_y):
    ...
    command = f'gnome-terminal -- bash -c "python3 localization.py {frame_center_x} {frame_center_y}; exec bash"'
    
    # Execute the command to open the terminal and run the script
    os.system(command)

def listen_for_rc_channels(connection, frame_center_x, frame_center_y):
    while True:
        # Receive RC channel data
        msg = connection.recv_match(type='RC_CHANNELS', blocking=True)
        if msg:
            # Extract channel values
            channel_15_value = msg.chan15_raw

            # Check if channel 15 is above the threshold
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                rc_thread = threading.Thread(target=listen_for_rc_channels, args=(connection, frame_center_x, frame_center_y))
                rc_thread.start()
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    #roll, pitch, yaw = calculate_camera_angles(frame_center_x, frame_center_y, img_width, img_height)
                    #print(f"Camera Angles: Roll = {roll:.2f}°, Pitch = {pitch:.2f}°, Yaw = {yaw:.2f}°")

                    fov_x = 2 * math.atan((sensor_width / 2) / focal_length_x) #* (180 / math.pi)  # Horizontal FOV in degrees
                    fov_y = 2 * math.atan((sensor_height / 2) / focal_length_y) #* (180 / math.pi) 
